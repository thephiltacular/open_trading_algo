...
class DatabasePopulator:
    def __init__(self, config_path=None, tickers_path=None, db_path=None):
        self.config_path = (
            Path(config_path)
            if config_path
            else Path(__file__).parent.parent / "config/live_data_config.yaml"
        )
        self.tickers_path = (
            Path(tickers_path)
            if tickers_path
            else Path(__file__).parent.parent / "config/all_tickers.yaml"
        )
        self.db = DataCache(db_path) if db_path else None
        self._load_config()
